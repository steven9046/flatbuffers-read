...
namespace tflite; // 命名空间

// This corresponds to the version.
file_identifier "TFL3"; // 这个东西是一个标记，用这个schema生成的所有序列化文件都会把这个标记写进去
// File extension of any written files.
file_extension "tflite"; // flatbuffers默认生成序列化文件的后缀是.bin，这里用这个参数可以改成任意的
...
table CustomQuantization {
  custom:[ubyte] (force_align: 16);//force_align 是一个属性，把这个obj强制对齐到16，这个是在设置占用内存的长度
}
...
// uninon 里可以装 table， table 里装的是基本的数据类型 constant 常量
...
// 每一个 builtin operator 都有一个对应的 option
// 一共120个??? 莫非有的operator没有对应的option?
...
// 每一个operator对应一个 code，因为不仅有builtin， 还有custom operator
// builtin operator 的 code 就是上边那个枚举
// cunstom operator 的 code 是一个 string
table OperatorCode {
...
// or custom_options.
// 操作符，通过 operator code 来找到对应的操作
table Operator {
  // Index into the operator_codes array. Using an integer here avoids
  // complicate map lookups.
  opcode_index:uint; // 这个就是对应的 operator code,如果是builtin，那就是上边那个140个enmu里的某一个数字，如果是custom，就是一个string
...
// 内存
table Buffer {
  data:[ubyte] (force_align: 16); // 一个 ubyte 的 array
}
...
table Model {
  ...
  // vector.
  // 这个模型里所有的 operator 的 code
  operator_codes:[OperatorCode]; // 很奇怪，之前确实遇到了这个 operator_codes 的顺序和 subgraph 里的operator的顺序不一样，莫名其妙
...
root_type Model;// 不对啊，这里定义的root type 是Model啊，上边说是subgraph，显然上边不对，这里代码写的肯定是正确的，上边只是注释里写的
